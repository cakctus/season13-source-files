...
void CObjectManager::ObjectStateProc(LPOBJ lpObj, int MessageCode, int aIndex, int SubCode) // OK
{
	...
	case 2:
		if (lpObj->Live != 0)
		{
			...
			if (GetLargeRand() % 100 >= gObj[aIndex].BackSpringProtection)
			{
...
bool CObjectManager::CharacterUseWarehouseExpansion(LPOBJ lpObj, CItem* lpItem) // OK
{
	...
	GCMessagePopupSend(lpObj, "Seu ba� extra foi criado com sucesso. Voc� precisa fazer o login novamente. Pressione ESC, em seguida Selecionar Servidor.");
...
void CObjectManager::CharacterCalcAttribute(int aIndex) // OK
{
	...
	if (lpObj->Map == MAP_CRYWOLF && Dexterity > maxDexterity[lpObj->Class])
	{
...
bool CObjectManager::CharacterInfoSet(BYTE* aRecv, int aIndex) // OK
{
	...
	if (lpObj->MapServerMoveRequest != 0)
	{
		...
		{
			//LogAdd(LOG_DEBUG, "N�O DEU PRA MOVER CS");
	...
	if (lpTarget->Life <= 0 && lpTarget->Live != 0)
	{
		...
		if (lpTarget->Type == OBJECT_USER)
		{
			...
			if (lpObj->Type == OBJECT_USER)
			{
				...
				if (lpTarget->Map == MAP_CRYWOLF) // && lpTarget->X >= 2 && lpTarget->X <= 62 && lpTarget->Y >= 2 && lpTarget->Y <= 62)
				{
