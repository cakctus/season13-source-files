...
void gObjMonsterDieGiveItem(LPOBJ lpObj, LPOBJ lpTarget) // OK
{
	...
	{
		gEffectManager.DelEffect(lpTarget, EFFECT_CHUCK_NORRIS);
		gEffectManager.AddEffect(lpTarget, 0, EFFECT_CHUCK_NORRIS, 86400, 0, 0, 0, 0);
	}
...
void gObjMonsterProcess(LPOBJ lpObj)
{
	...
	if (lpObj->Class == 746)
	{
		...
		{
			//LogAdd(LOG_BLACK, "[NixiesBoss] NixiesBoss does not exist");
			return;
		}
		...
		{
			//LogAdd(LOG_DEBUG, "Return %s %d", __FUNCTION__, __LINE__);
			return;
		}
	}
