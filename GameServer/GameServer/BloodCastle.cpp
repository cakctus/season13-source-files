...
void CBloodCastle::SetState_EMPTY(BLOOD_CASTLE_LEVEL* lpLevel) // OK
{
	...
	{
		LogAdd(LOG_RED, "gObjDel %s %s %d [Class: %d]", __FILE__, __FUNCTION__, __LINE__, gObj[lpLevel->GateIndex].Class);
	...
	{
		LogAdd(LOG_RED, "gObjDel %s %s %d [Class: %d]", __FILE__, __FUNCTION__, __LINE__, gObj[lpLevel->SaintStatueIndex].Class);
...
void CBloodCastle::SetState_CLEAN(BLOOD_CASTLE_LEVEL* lpLevel) // OK
{
	...
	for(int n=0;n < MAX_BC_USER;n++)
	{
		...
		{
			//TODO - config to reward only winner and not party			
			if(lpLevel->User[n].Index == lpLevel->WinnerIndex || 
				(gServerInfo.m_BloodCastleRewardParty == 1 && 
					OBJECT_RANGE(lpLevel->WinnerIndex) != 0 && 
					gParty.IsMember(gObj[lpLevel->WinnerIndex].PartyNumber,lpLevel->User[n].Index) != 0)
				)
			{
...
void CBloodCastle::SetMonster(BLOOD_CASTLE_LEVEL* lpLevel,int MonsterClass) // OK
{
	for(int n=0;n < gMonsterSetBase.m_count;n++)
	{
		...
		if(OBJECT_RANGE(index) == 0)
		{
			if (MonsterClass == 143)
			{
				LogAdd(LOG_RED, "[BC] Line %d, Var %d", __LINE__, index);
			}
		...
		if(gObjSetPosMonster(index,n) == 0)
		{
			if (MonsterClass == 143)
			{
				LogAdd(LOG_RED, "[BC] Line %d, Var %d %d", __LINE__, index, n);
			}
			LogAdd(LOG_RED, "gObjDel %s %s %d [Class: %d]", __FILE__, __FUNCTION__, __LINE__, gObj[index].Class);
			gObjDel(index);
			continue;
		...
		if(gObjSetMonster(index,lpInfo->MonsterClass) == 0)
		{
			if (MonsterClass == 143)
			{
				LogAdd(LOG_RED, "[BC] Line %d, Var %d %d", __LINE__, index, lpInfo->MonsterClass);
			}
			LogAdd(LOG_RED, "gObjDel %s %s %d [Class: %d]", __FILE__, __FUNCTION__, __LINE__, gObj[index].Class);
			gObjDel(index);
			continue;
		...
		if(this->AddMonster(lpLevel,index) == 0)
		{
			if (MonsterClass == 143)
			{
				LogAdd(LOG_RED, "[BC] Line %d, Var %d %d", __LINE__, index, lpLevel->Level);
			}
			LogAdd(LOG_RED, "gObjDel %s %s %d [Class: %d]", __FILE__, __FUNCTION__, __LINE__, gObj[index].Class);
			gObjDel(index);
			continue;
...
void CBloodCastle::MonsterDieProc(LPOBJ lpObj, LPOBJ lpTarget) // OK
{
	...
	//TODO add log pra drop ou não drop do item da estatua
...
void CBloodCastle::CGBloodCastleEnterRecv(PMSG_BLOOD_CASTLE_ENTER_RECV* lpMsg,int aIndex) // OK
{
	...
	{
		GCMessagePopupSend(lpObj, "Blood Castle esta disponivel apenas para usuarios VIP.");
		return;
	}*/
