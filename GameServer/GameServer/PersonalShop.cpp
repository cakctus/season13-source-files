...
void CPersonalShop::CGPShopBuyItemRecv(PMSG_PSHOP_BUY_ITEM_RECV* lpMsg,int aIndex) // OK
{
	...
	if (lpObj->PersonalCodeChecked == false)
	{
		GCMessagePopupSend(lpObj, "Você não pode comprar itens de loja pessoal sem antes autenticar o PIN. Utilize o comando /pin seguido de seu PIN para faze-lo agora mesmo.");
		return;
	}
...
void CPersonalShop::OnPShopBuyItemCallbackRecv(LPOBJ lpObj, LPOBJ lpTarget, DWORD slot, DWORD WCoinC, DWORD WCoinP, DWORD GoblinPoint) // OK
{
	...
	gLog.Output(LOG_TRADE, "[SellPesonalShopItem][%s][%s] - (Account: %s, Name: %s, Value: %d, WCValue: %d, Index: %d, TimerRandom: %02d, Serial: %08X, Skill: %01d, Option2: %01d, Option3: %01d, NewOption: %03d, JewelOfHarmonyOption: %03d, ItemOptionEx: %03d, SocketOption: %03d, %03d, %03d, %03d, %03d)", 
		lpTarget->Account, lpTarget->Name, lpObj->Account, lpObj->Name, 0, PShopWCCValue, lpTarget->Inventory[slot].m_Index, lpTarget->Inventory[slot].m_Level, lpTarget->Inventory[slot].m_Serial, lpTarget->Inventory[slot].m_Option1, lpTarget->Inventory[slot].m_Option2, lpTarget->Inventory[slot].m_Option3, lpTarget->Inventory[slot].m_NewOption, lpTarget->Inventory[slot].m_JewelOfHarmonyOption, lpTarget->Inventory[slot].m_ItemOptionEx, lpTarget->Inventory[slot].m_SocketOption[0], lpTarget->Inventory[slot].m_SocketOption[1], lpTarget->Inventory[slot].m_SocketOption[2], lpTarget->Inventory[slot].m_SocketOption[3], lpTarget->Inventory[slot].m_SocketOption[4]);

	GDItemTrackingSaveSend(lpTarget->Inventory[slot], lpTarget->Inventory[slot], lpTarget->Account, lpObj->Account, 3, 2);

	gCashShop.GDCashShopSubPointSaveSend(lpObj->Index, 0, PShopWCCValue, 0, 0, "PersonalStore");
	...
	gCashShop.GDCashShopAddPointSaveSend(lpTarget->Index, 0, PShopWCCValue, 0, 0, "PersonalStore");
